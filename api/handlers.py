...
from api.actions.auth import get_current_active_user
...
async def delete_user(
    ...
    deleted_user_id = await _delete_user(current_active_user.user_id, db)
    if deleted_user_id is None:
        raise HTTPException(
            status_code=404,
            detail=f"User with id {current_active_user.user_id} not found.",
        )
